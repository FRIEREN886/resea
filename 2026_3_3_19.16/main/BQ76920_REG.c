...
{
    //将P405切换成输出模式
     R_IOPORT_PinCfg (&g_ioport_ctrl,BSP_IO_PORT_04_PIN_05, IOPORT_CFG_PORT_DIRECTION_OUTPUT);
}

static void BQ769X0_TS1_SetInMode(void)
{
    //将P405切换成输入模式
     R_IOPORT_PinCfg (&g_ioport_ctrl,BSP_IO_PORT_04_PIN_05,IOPORT_CFG_PORT_DIRECTION_INPUT);
}

void BQ769X0_ALERT_IRQ10_Init()
{
    fsp_err_t err = FSP_SUCCESS;
    err = R_ICU_ExternalIrqOpen(&BQ769X0_ALERT_IRQ10_ctrl, &BQ769X0_ALERT_IRQ10_cfg);
    err = R_ICU_ExternalIrqEnable(&BQ769X0_ALERT_IRQ10_ctrl);
    assert(FSP_SUCCESS == err);
}

void BQ769X0_ALERT_IRQ10_Callback(external_irq_callback_args_t *p_args)
{
